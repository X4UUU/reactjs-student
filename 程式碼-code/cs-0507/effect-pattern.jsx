...
export default function EffectPattern() {
  ...
  //說明:用於需要紀錄，或是一些特殊專用的勾子開發時
  //使用情況:一般應用程式開發少用到
  useEffect(() => {
    // console.log('每次渲染(render)之後(after)執行一次');
  })
  //樣式2:首次渲染(render)之後(after)執行一次，之後不會再執行
  //說明:近似於生命週期方法中的componentDidMount(didMount)
  //使用情況:開發應用時最常用到的樣式。用於元件首次呈現後，與伺服器fetch/ajax獲取資料，或與第三方應用整合。
  useEffect(() => {
    console.log('每次渲染(render)之後(after)執行一次，之後不會再執行')
  }, [])
  ...
  useEffect(() => {
    console.log(
      '每次渲染(render)之後(after)執行一次。之後當相依變數有更動之後(after)，又會執行一次'
    )
  }, [total])
  //^^^^^^^ total加入倒相依變數陣列中，類似要監聽total狀態的更動(change)事件
  //注意:因為useEffect與render有絕對關係，所能加入到相依變數中的變數，必然要和re-render(update)有關
  //因此只有props或state(或兩者衍生的變數)才能加到相依性陣列
  return (
    <>
